...
namespace Nop.Services.ExportImport
{
    ...
    public partial class ImportManager : IImportManager
    {
        ...
        /// <summary>
        /// Import Product Attribute from Excel
        /// </summary>
        /// <param name="stream"></param>
        public virtual void ImportProductsAttributeFromXlsx(Stream stream)
        {
            using (var xlPackage = new ExcelPackage(stream))
            {
                // get the first worksheet in the workbook
                ...
                var propertiesMapping = new []
                {
                 ...
                 while (true)
                 {
                     ...
                     if (allColumnsAreEmpty)
                         break;
                     int ProductId = Convert.ToInt32(worksheet.Cells[iRow, GetColumnIndex(propertiesMapping, "ProductId")].Value);
                     int ProductAttributeId = Convert.ToInt32(worksheet.Cells[iRow,GetColumnIndex(propertiesMapping,"ProductAttributeId")].Value);
                     string TextPrompt = Convert.ToString(worksheet.Cells[iRow, GetColumnIndex(propertiesMapping, "TextPrompt")].Value);
                     bool IsRequired = Convert.ToBoolean(worksheet.Cells[iRow, GetColumnIndex(propertiesMapping, "IsRequired")].Value);
                     int AttributeControlTypeId = Convert.ToInt32(worksheet.Cells[iRow, GetColumnIndex(propertiesMapping, "AttributeControlTypeId")].Value);
                     int DisplayOrder = Convert.ToInt32(worksheet.Cells[iRow, GetColumnIndex(propertiesMapping, "DisplayOrder")].Value);
                     int ValidationMinLength = Convert.ToInt32(worksheet.Cells[iRow, GetColumnIndex(propertiesMapping, "ValidationMinLength")].Value);
                     int ValidationMaxLength = Convert.ToInt32(worksheet.Cells[iRow, GetColumnIndex(propertiesMapping, "ValidationMaxLength")].Value);
                     string ValidationFileAllowedExtensions = Convert.ToString(worksheet.Cells[iRow, GetColumnIndex(propertiesMapping, "ValidationFileAllowedExtensions")].Value);
                     int ValidationFileMaximumSize = Convert.ToInt32(worksheet.Cells[iRow, GetColumnIndex(propertiesMapping, "ValidationFileMaximumSize")].Value);
                     string DefaultValue = Convert.ToString(worksheet.Cells[iRow, GetColumnIndex(propertiesMapping, "DefaultValue")].Value);
                     ...
                     var prodcutAttributeMapping = new ProductAttributeMapping();
                     prodcutAttributeMapping.ProductId = ProductId;
                     prodcutAttributeMapping.ProductAttributeId = ProductAttributeId;
                     prodcutAttributeMapping.TextPrompt = TextPrompt;
                     prodcutAttributeMapping.IsRequired = IsRequired;
                     prodcutAttributeMapping.AttributeControlTypeId = AttributeControlTypeId;
                     prodcutAttributeMapping.DisplayOrder = DisplayOrder;
                     prodcutAttributeMapping.ValidationMinLength = ValidationMinLength;
                     prodcutAttributeMapping.ValidationMaxLength = ValidationMaxLength;
                     prodcutAttributeMapping.ValidationFileAllowedExtensions = ValidationFileAllowedExtensions;
                     prodcutAttributeMapping.ValidationFileMaximumSize = ValidationFileMaximumSize;
                     prodcutAttributeMapping.DefaultValue = DefaultValue;

                     _productAttributeService.InsertProductAttributeMapping(prodcutAttributeMapping);


                     int AttributeValueTypeId = Convert.ToInt32(worksheet.Cells[iRow, GetColumnIndex(propertiesMapping, "AttributeValueTypeId")].Value);
                     int AssociatedProductId = Convert.ToInt32(worksheet.Cells[iRow, GetColumnIndex(propertiesMapping, "AssociatedProductId")].Value);
                     string Name = Convert.ToString(worksheet.Cells[iRow, GetColumnIndex(propertiesMapping, "Name")].Value);
                     string ColorSquaresRgb = Convert.ToString(worksheet.Cells[iRow, GetColumnIndex(propertiesMapping, "ColorSquaresRgb")].Value);
                     decimal PriceAdjustment = Convert.ToDecimal(worksheet.Cells[iRow, GetColumnIndex(propertiesMapping, "PriceAdjustment")].Value);
                     decimal WeightAdjustment = Convert.ToDecimal(worksheet.Cells[iRow, GetColumnIndex(propertiesMapping, "WeightAdjustment")].Value);
                     decimal Cost = Convert.ToDecimal(worksheet.Cells[iRow, GetColumnIndex(propertiesMapping, "Cost")].Value);
                     int Quantity = Convert.ToInt32(worksheet.Cells[iRow, GetColumnIndex(propertiesMapping, "Quantity")].Value);
                     bool IsPreSelected = Convert.ToBoolean(worksheet.Cells[iRow, GetColumnIndex(propertiesMapping, "IsPreSelected")].Value);
                    // int DisplayOrder = Convert.ToInt32(worksheet.Cells[iRow, GetColumnIndex(propertiesVaules, "DisplayOrder")].Value);
                     int PictureId = Convert.ToInt32(worksheet.Cells[iRow, GetColumnIndex(propertiesMapping, "PictureId")].Value);
                   
                     var pm = _productAttributeService.GetProductAttributeMappingsByProductId(ProductId);
                     var productAttributeValue = new ProductAttributeValue();
                     
                     productAttributeValue.AttributeValueTypeId = AttributeControlTypeId;
                     productAttributeValue.AssociatedProductId = AssociatedProductId;
                    // productAttributeValue.Name = Name;
                     productAttributeValue.ColorSquaresRgb = ColorSquaresRgb;
                     productAttributeValue.PriceAdjustment = PriceAdjustment;
                     productAttributeValue.WeightAdjustment = WeightAdjustment;
                     productAttributeValue.Cost = Cost;
                     productAttributeValue.Quantity = Quantity;
                     productAttributeValue.IsPreSelected = IsPreSelected;
                     productAttributeValue.DisplayOrder = DisplayOrder;
                     productAttributeValue.PictureId = PictureId;
                     string[] names = Name.Split(',');
                     foreach (var p in pm)
                     {
                         if(p.ProductAttributeId == ProductAttributeId )
                         {
                             productAttributeValue.ProductAttributeMappingId = p.Id;
